...
import { Runtime, type RuntimeService } from "@/application/runtime-context";
...
export function makePluginsLive(
  ...
  return Layer.effect(
    ...
    Effect.gen(function* () {
      ...
      const pluginManagement = yield* tryApplicationSync(
        () =>
      ...
      return makePluginsService({
        ...
        getPluginManagement: () => Effect.succeed(pluginManagement),
        commands,
      });
    }),
  );
...
export function makePluginsService(dependencies: PluginsServiceDependencies): PluginsService {
    ...
    dependencies
      ...
      .pipe(
        Effect.flatMap((management) =>
          tryApplicationSync(() => management.listPurgeablePluginData()),
        ),
      );
  ...
  return Object.freeze({
    ...
    setEnabled: (
      ...
      Effect.gen(function* () {
        ...
        const management = yield* dependencies.getPluginManagement();
    ...
    uninstall: (pluginId: string, request: ApplicationPluginLocaleRequest = {}) =>
      Effect.gen(function* () {
        ...
        const uninstallExit = yield* Effect.exit(
          tryApplicationPromise(async () => management.uninstall(pluginId)),
        );
    ...
    purgeData: (pluginId: string): ApplicationEffect<ApplicationPluginPurgeResult> =>
      Effect.gen(function* () {
        ...
        const management = yield* dependencies.getPluginManagement();
        const purged = yield* management.purge(pluginId);
