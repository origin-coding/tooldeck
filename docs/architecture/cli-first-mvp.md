...
- `internal/runtime-node` 是当前 Node runtime 的 TypeScript 实现，负责 manifest indexing、command orchestration、command result validation、runtime-kind routing 和 Node plugin loading。
- `packages/sdk-node` 负责公开的 plugin authoring APIs 和插件作者契约。
- `packages/storage` 负责 SQLite schema、migrations、repositories。
- `apps/cli` 组合 runtime-node、storage 和本地插件。
